package com.walterjwhite.examples.practice.problems.trees;
...
public class Trees {
  ...
  public void printLevel(final int level, final Node... nodes) {
    ...
    System.out.println(String.format("level: %d %d", level, sum));

    if (i > 0) {
      printLevel(level + 1, children);
    } else {
      System.out.println("Exiting as no nodes were found on this level.");
    }
  }
}
